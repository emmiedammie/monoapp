...
/**
 * Performance test for the Carer entity.
 */
class CarerGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Carer entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all carers")
            .get("/api/carers")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new carer")
            .post("/api/carers")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "name":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_carer_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created carer")
                .get("${new_carer_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created carer")
            .delete("${new_carer_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
