...
/**
 * Performance test for the Rota entity.
 */
class RotaGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Rota entity")
        ...
        .pause(10)
        .repeat(2) {
            exec(http("Get all rotas")
            .get("/api/rotas")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new rota")
            .post("/api/rotas")
            .headers(headers_http_authenticated)
            .body(StringBody("""{
                "client":"SAMPLE_TEXT"
            ...
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_rota_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created rota")
                .get("${new_rota_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created rota")
            .delete("${new_rota_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
