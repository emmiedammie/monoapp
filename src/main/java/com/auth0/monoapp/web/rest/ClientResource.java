...
/**
 * REST controller for managing {@link com.auth0.monoapp.domain.Client}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ClientResource {
        ...
        Optional<Client> result = clientRepository
            ...
            .map(existingClient -> {
                ...
                if (client.getAccesscode() != null) {
                    existingClient.setAccesscode(client.getAccesscode());
                }
                if (client.getTask() != null) {
                    existingClient.setTask(client.getTask());
                }
                if (client.getCarerassigned() != null) {
                    existingClient.setCarerassigned(client.getCarerassigned());
                }
    ...
    public List<Client> getAllClients(@RequestParam(required = false) String filter) {
        if ("visit-is-null".equals(filter)) {
            log.debug("REST request to get all Clients where visit is null");
            return StreamSupport
                .stream(clientRepository.findAll().spliterator(), false)
                .filter(client -> client.getVisit() == null)
